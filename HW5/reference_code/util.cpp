...
double Combination(int n, int k) {
  if (n < k || k < 0) {
    throw invalid_argument(
        "Comination: the number of elements should be bigger than selection "
        "numbers AND two numbers should be positive\n");
    return EXIT_FAILURE;
  }
  double result = 1e308;
  if (pow(result, 1.0 / k) < n) {
    throw invalid_argument("The Combination number may be bigger than the "
                           "maxium of double precision\n");
    return EXIT_FAILURE;
  }
  double n_d = (double)n;
  result = 1.0;
...
double Overlap1st_onedim(double xa, double xb, double alphaa, double alphab,
                         int la, int lb) {
  // double x = Combination(3, 1);
  // double y = DoubleFactorial(4);
...
void Overlap1st_3d(arma::vec &S_Ra, arma::vec &Ra, arma::vec &Rb, double alphaa,
                   double alphab, arma::uvec &lmna, arma::uvec &lmnb) {
...
void I2e_pG_1st(arma::vec &gammaAB_Ra, arma::vec &Ra, arma::vec &Rb,
