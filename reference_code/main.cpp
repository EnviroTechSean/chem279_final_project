...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    // arma::vec center = mol.mAtoms[0].mAOs[0].get_center();
    // center(0) += 1e-5;
    // mol.mAtoms[0].set_center(center);
    // mol.eval_gammamat(gamma_mat);
    ...
    printf("\nIf I use finite difference, change the x coordinate of second "
           "atom:\n");

    printf("The molecule in file %s has energy %f\n", argv[1], Energy);
    arma::vec center = mol.mAtoms[1].mAOs[0].get_center();
    center(0) += 1e-6;
    mol.mAtoms[1].set_center(center);
