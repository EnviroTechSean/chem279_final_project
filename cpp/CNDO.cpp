...
std::map<std::string, CNDO_para> CNDO_para_map{
    {"H", H_para}, {"C", C_para}, {"N", N_para}, {"O", O_para}, {"F", F_para},
};

CNDO::CNDO(Molecule_basis &mol_i, int max_it, double tolerence)
    : mol(mol_i), max_iter(max_it), tol(tolerence) {
  dim = mol.getnumAOs();
      ...
      for (size_t j = 0; j < num_atoms; j++) {
        ...
        for (auto ao : mol.mAtoms[j].mAOs) {
          if (k_AO != j_AO) {
            ...
            // cout << k_AO << "  " << j_AO<< "  " << gammaAB<<"  " << Pa(k_AO,
            // j_AO)<< "  " << Ga(k_AO, j_AO)<< endl;
          }
          j_AO++;
        }
      }
...
int CNDO::run() {
  ...
  if (k == max_iter) {
    cout << "Error: the job could not be finished in " << max_iter
         << "iterations.\n";
    return 1;
  }
  // Ea.print("Ea");
    ...
    for (size_t j = 0; j < num_atoms; j++) {
      ...
      for (size_t k_AO = k_AO_off; k_AO < k_AO_off + A_Atom.mAOs.size();
        ...
        for (size_t j_AO = j_AO_off;
          ...
          gradient_A -= (Pa(k_AO, j_AO) * Pa(k_AO, j_AO) +
